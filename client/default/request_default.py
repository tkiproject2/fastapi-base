...
async def getRequestClient(
    url: str,
    param: dict = None,
    headers: dict = None,
    httpBasic: ClientHttpBasicAuth = None,
) -> ClientResponseModel:
    ...
    if httpBasic:
        async with aiohttp.ClientSession(headers=headers).auth(
            BasicAuth(httpBasic.username, httpBasic.password)
        ) as session:
            try:
